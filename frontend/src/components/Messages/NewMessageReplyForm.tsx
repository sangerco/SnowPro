...
const NewMessageReplyForm: React.FC<NewReplyProps> = ({
  newReply,
  error,
  sendNewReplyData,
}) => {
  ...
  useEffect(() => {
    if (messageData) {
      setFormData((prevFormData) => ({
        ...
        sender_id:
          messageData.recipient_id /* vital that sender_id and recipient_id are switched here */,
  ...
  return (
    <Form onSubmit={handleSubmit} error>
      <Form.Field>
        <input
          placeholder={`Re: ${formData.subject}`}
          name="subject"
...
export default connect(mapStateToProps, { sendNewReplyData })(
  NewMessageReplyForm
);
