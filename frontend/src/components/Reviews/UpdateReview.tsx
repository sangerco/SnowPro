...
import {
  updateReview,
  fetchReviewDataById,
} from "../../oldRedux/actions/reviewActions";
  ...
  useEffect(() => {
    const fetchData = async (slug: string, id: string) => {
      try {
        ...
        return (
          <p>{`Review could not be retrieved! Error: ${error.message}`}</p>
        );
      }
    };
    fetchData(slug || "", id || "");
  ...
  return (
    <Container fluid>
      <Form onSubmit={handleSubmit} error>
        ...
        <Button type="submit" red onClick={handleCancel}>
          Cancel
        </Button>
      ...
      <Button onClick={() => setShowStarsDropdown(true)} green>
        Change rating?
      </Button>
      {showStarsDropdown && (
        <Dropdown
