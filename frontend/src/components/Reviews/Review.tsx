  ...
  if (review) {
    ...
    return (
      <div>
        ...
        <Container textAlign="left">
          <Header as="h5">{review.skiAreaName}</Header>
        </Container>
        <Container textAlign="left">
        ...
        {review.replyData.length > 0 ? (
          review.replyData.map((reviewReply) => (
            <ReviewReply key={reviewReply.id} reviewReply={reviewReply} />
          ))
        ) : (
          <Divider />
        )}

        <Button onClick={() => setShowReviewReplyForm(true)}>
